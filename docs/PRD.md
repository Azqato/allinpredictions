...
| **New:** Full retroactive `resolves_by` backfill across all pre-existing `inconclusive` checks (~685 remaining) | Explicitly deferred, user-requested 2026-08-31: "do one batch as a test example... do not do this today but mark it on the roadmap as a full refresh we need to do eventually" | Planned, not started. Read each prediction's original timeframe/premise and assign a `resolves_by` date, in batches of 5 at a time (per the test batch above), until all inconclusive checks carry a resolution-date expectation |
| Curated "Big Ones" high-impact section (item 3) and sitewide search (item 4), done together | Ranked #3 and #4 by effort in the same 2026-08-31 prioritization, user-requested to do both in one go | **Complete 2026-08-31** - (3) `config/big_ones.json` lists 12 hand-picked `{episode_id, id}` pairs, resolved against live prediction/check data at build time and rendered as a new home page section between the headline stat and Host Accuracy; mix of 5 confirmed-right, 5 confirmed-wrong, and 2 still-inconclusive marquee predictions (crypto market cap, Sam Altman/OpenAI board crisis, "President Kamala for 4 months", Lutnick's $1T DOGE pledge, Gerstner's $2T SpaceX IPO call, Twitter valuation, university-president resignations, Anthropic $100B ARR, election-fraud claim, Ukraine collapse call, plus both of today's new predictions). (4) New `search.html` + `static/search_index.json`: single search box across all 4,001 predictions/episodes/hosts/guests, type-filterable, results render only once the visitor types (avoids dumping the full index). Both reuse the existing `prediction-card`/`badge-*` CSS patterns, no new styling needed. Site rebuilt clean |
