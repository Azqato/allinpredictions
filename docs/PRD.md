...
- `generate_site.py` has no data-validation layer (§28) - it assumes `data/predictions/*.json`/`data/checks/*.json` already match the §7 schemas. The freeberg/unknown-role bug fixed on 2026-08-25 (a bad `role: "host"` on a misspelled `who`, bypassing the `permanent_hosts` check in `build_speaker_index()`) is a direct example of this gap causing a real, live, user-visible bug rather than a caught error. This is a known, still-open risk, not just a historical incident - the same class of bug could recur with any future hand-edited or migration-script-written prediction file.
...
**Work in progress as of 2026-08-25:** none - `git status` is clean and there are no unmerged branches (single-branch `main` history, per §28). The full-archive validation sweep (32/357 episodes validated) and the `video_id` resolution backlog (45 episodes) are ongoing, tracked work, not uncommitted/stubbed code.
...
3. Is `data/manifest.json`'s `generated_at` timestamp (`2026-08-05T05:03:49Z` as of this audit) expected to be refreshed every time `build_manifest.py` runs, including runs that only touch a handful of episodes? It currently looks stale relative to the 2026-08-25 validation batch and site restructure, which suggests either `build_manifest.py` wasn't re-run after those changes, or its `generated_at` field isn't being treated as load-bearing. Not urgent, but worth a definitive answer so a future session doesn't misread it as "the archive hasn't been touched since 2026-08-05."
