...
| **New:** Host/guest name-accuracy audit - verify every `who` slug across `data/predictions/` maps to a real, correctly spelled "First Name Last Name" (permanent hosts already use their canonical short slugs per `config/hosts.yaml`; this targets guest slugs and display names, which should never be a bare first name, a bare last name, a nickname, or a misspelling) | Post-launch, after the incremental-process backlog is complete (satisfied 2026-08-27) | Planned, added to the roadmap 2026-08-27 at user request |
