...
- Full retroactive `resolves_by` backfill and the ongoing full-archive validation sweep (both above in the main §19 table) are independent maintenance tracks with no dependency relationship to this UI work - nothing here blocks them and they block nothing here, so they're left at their existing, user-set cadence rather than folded into this dependency sort.
