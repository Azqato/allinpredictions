...
| §35: Listener voting/feedback mechanism (Reddit feedback, 2026-08-27) | Resolved 2026-08-31 as a standing architecture decision (no longer treated as deferred) | **Complete 2026-08-31** - implemented via a footer link to `github.com/Azqato/allinpredictions/issues` on every page, per explicit user decision to use GitHub Issues rather than build a custom voting backend, so this site collects no user data itself. This is the permanent answer, not a stopgap - see §35 for full reasoning |
...
| Combined the `resolves_by` backfill into the full-archive validation sweep row; added a footer link to GitHub Issues, then decided it *is* the permanent §35 listener-voting/feedback answer (not a stopgap) | Same-day follow-up requests, 2026-08-31: "combine this one into full-archive validation and make its status as planned" / "lets point people towards this page: github.com/Azqato/allinpredictions/issues" / follow-up decision: "the full listener-voting feature should be part of the issues submission. its no longer deferred that's the decision i made moving forward so we dont colelct user data" | **Complete 2026-08-31** - merged the two validation-sweep rows into one "Planned, continuing indefinitely as maintenance" row. Added a footer link on every page ("Open an issue on GitHub") pointing to the repo's public/enabled-by-default Issues tab. Initially framed as a lightweight substitute alongside a still-deferred full voting feature; the user then made the explicit call that GitHub Issues *is* the listener-voting mechanism going forward, specifically to avoid this site collecting any user data itself - §35 rewritten from "Deferred" to "Complete" accordingly |
...
| §36.1-36.5: visual/UX detail re-audit (hero block, per-host highlight cards, "Fresh Verdicts" feed, "The Standings" leaderboard, "The Big Ones" curated section, "This Episode's Calls", topic tag cloud), a new "The Roster" host+guest index page sorted by prediction count, a new "The Full Ledger" browse/search/filter page, a redesigned Methodology page, and a sitewide distinct-naming convention | Same sequencing as §36 (after the full ingest-and-validation sweep); documentation-only pass 2026-08-27, no implementation yet | **Split into 11 tracked subtasks 2026-08-31, re-sequenced by dependencies + effort the same day** (see §19.1 below) - broken down at user request ("can we split #2 up into subtasks?"), then resequenced at follow-up request ("sort the roadmap based on dependencies and what requires the least effort first"). §19.1 items 1-5 complete 2026-08-31 (see row below); items 6-11 not started, §19.1 is the authoritative task list and execution order for the rest |
| §19.1 items 1-5: verdict data-model decision, reusable prediction-card component, home page Topic Index tag cloud, hero block, "This Episode's Calls" section | Dependency-tier-first order established by the 2026-08-31 re-sequencing above; user-requested 2026-08-31 ("do #1-5 in one pass, update documentation and the push to production") | **Complete 2026-08-31** - `ambiguous`/`inconclusive` now display sitewide as "Partly Right"/"Too Early" (data model unchanged, display-only via `verdict_display()`/`verdictDisplay()`); new `_prediction_card.html` macro (`card_link`/`card_article`) now backs episode.html, host.html, and the home page's Big Ones/Recently Settled sections; home page gained a Topic Index pill row (live per-topic counts, click-to-filter), a rewritten hero block ("Did they call it?" + stat/CTA line), and a "This Episode's Calls" section (latest episode's qualifying predictions, capped at 6) placed after Recently Settled |
| Moved the home page Topic Index above Guest Predictions | Same-day follow-up request, 2026-08-31: "can you move the 'Topic Index' to above the 'Guest Predictions' section?" | **Complete 2026-08-31** - relocated from between the hero and Host Accuracy into the Host Accuracy section, directly after the host scorecards and before Guest Predictions |
| §19.1 item 6: Methodology page redesign | Next in dependency/effort order after items 1-5; user-requested 2026-08-31 ("what's next?" -> "yes") | **Complete 2026-08-31** - `about.html` rewritten with an eyebrow label, original headline/subhead, a numbered five-step pipeline mapped onto §6, a five-row verdict glossary, and a new section stating guest predictions are scored too (point of difference from other trackers). Added a footer link to it on every page, since it had no inbound link anywhere after "About" left the nav |
| Footer width fix + sitemap | User-requested 2026-08-31 ("can you please fix the footer's width? and also add a sitemap to the footer? similar to how composeratlas.com manages its sitemap/footer"), referencing `github.com/Azqato/composer` | **Complete 2026-08-31** - the footer's paragraphs were capped at a cramped `60ch` inside the 960px `.container`; restructured to composeratlas.com's pattern: a centered, uncapped sitemap link row (every top-level page + Feedback), then a wider (`720px`) legal line, then a short copyright line |
| Added "About" to the footer sitemap | Same-day follow-up request, 2026-08-31: "we can add the About to the sitemap footer now too" | **Complete 2026-08-31** - added an "About" link next to "Methodology" in the footer sitemap; both point to `about.html` (the same page since the §19.1 item 6 redesign merged About into Methodology), but "About" jumps to a new `#about-this-project` anchor on that page instead of duplicating a plain top-of-page link |
...
|---|---|---|---|---|
| 1 | Verdict data-model decision: resolve whether `ambiguous`/`inconclusive` map cleanly to "partly right"/"too early," or need reframing/a new state, before any accuracy or leaderboard visual locks in a denominator (§36.1 item 8, §36.4) | None | Low | Complete 2026-08-31 |
| 2 | Reusable prediction-card component: one markup/JS pattern for a prediction card, used by every section/page that renders one (home sections, Full Ledger, episode pages) instead of near-duplicate card HTML per template (per §36.3's note) | None | Medium | Complete 2026-08-31 |
| 3 | "Topic Index" tag cloud with live per-topic counts on the home page (§36.1 item 7) | #2 | Low | Complete 2026-08-31 |
| 4 | Home page hero block: bold two-line headline + one-line sitewide stat as a CTA (§36.1 item 1) | #1 | Low-Medium | Complete 2026-08-31 |
| 5 | "This Episode's Calls": every call from the single most-recent episode (including ungraded), distinct from the existing Recently Settled section (§36.1 item 6) | #2 | Low-Medium | Complete 2026-08-31 |
| 6 | Methodology page redesign: numbered pipeline steps (§6) + a verdict glossary, explicitly noting guest predictions are scored too (a stated point of difference from the competitor) (§36.4) | #1 | Medium | Complete 2026-08-31 |
...
6. *What does "Resolved only" mean?* It shows accuracy as right-vs-wrong only, excluding predictions still too early to judge (`inconclusive`, displayed as "Too Early") or that landed as a mixed/partial outcome (`ambiguous`, displayed as "Partly Right") from the percentage.
...
## 35. Listener Voting / Feedback Mechanism

**Status:** Complete 2026-08-31 - resolved as a standing architecture decision, not deferred. The user made the explicit call to implement this via GitHub Issues rather than build a custom voting backend: "the full listener-voting feature should be part of the issues submission. its no longer deferred that's the decision i made moving forward so we dont colelct user data." A footer link on every page ("Spot a bad call, a wrong attribution, or have a feature idea? Open an issue on GitHub") points to `https://github.com/Azqato/allinpredictions/issues`. This *is* the listener voting/feedback mechanism now, not a stopgap ahead of a bigger build: a listener who disagrees with a house verdict opens an issue and makes their case, rather than casting an anonymous vote this site tallies itself.

**Context:** the original Reddit feedback thread asked for a way for listeners to vote or otherwise register their own opinion on a prediction's outcome, as a check on (or supplement to) the site's own "house" verdict (`right`/`wrong`/`ambiguous`/`inconclusive`, §7, produced by the validation pipeline in §6).

**Why GitHub Issues, and why this is the final answer, not a placeholder:** this site is deliberately static and accountless - no backend, no database, no user auth (§1 Goals, §23 Security explicitly states both are "none" by design). Every custom-voting candidate this section previously considered (a free-tier serverless function + KV store, a third-party embeddable widget) means this site collecting and durably storing per-visitor data itself, plus defending it against ballot-stuffing with no accounts to rate-limit against - exactly the privacy posture the user decided against ("so we dont colelct user data"). GitHub Issues sidesteps all of it: GitHub owns the account system, spam/abuse handling, and data storage; this site stores nothing new and adds no backend. The trade-off - a GitHub account is required to weigh in, and it's a discussion thread rather than a tallied vote count - was made deliberately in exchange for zero data collection, not overlooked.

**No further trigger or follow-up scheduled.** This closes out §35; nothing here depends on §13/§19 completion status the way the original "revisit later" framing assumed.
